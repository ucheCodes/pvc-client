<script setup lang="ts">
    ...
    onMounted(() => {
      console.log(usersArray.value.length);
    })
</script>
<template>
    <div>
        <h1>E - PVC Dapp registered users</h1>
        <div>
        <table class="table">
          <thead>
            <th>S / N</th>
          ...
          <tbody>
            <tr v-for="(user, count) in usersArray" :key="user.walletAddress">
              ...
              <td data-label="Voter's Address">{{user.walletAddress}}</td>
              ...
              <!-- <td data-label="has voted?">{{user.hasVoted}}</td> -->
            </tr>
          </tbody>
        </table>
      </div>
